...
function resize() {
  ...
  if (w !== canvas.width || h !== canvas.height) {
    ...
    // -----------------
    var strokeElements = [].slice.call(document.querySelectorAll('.stroke'));

    strokeElements.forEach(function (el) {
      el.addEventListener('click', function () {
        var stroke = parseInt(this.getAttribute('data-stroke'), 10);
        ctx.lineWidth = stroke;
        document.querySelector('.selected-stroke').classList.remove('selected-stroke');
        this.classList.add('selected-stroke');
      });
    });
    // ----------------------
  };
