...
span.innerHTML = 'Aplicación creada por Raúl'
...
//   //    6) En función del atributo 'complete' del objeto ToDo recibido como argumento:
//   todo.complete && toDoText.classList.add('completeText');
...
function displayToDos() {
  ...
  // Setear el innerHTML de 'toDoContainer' como un string vacio ("")
  toDoContainer.innerHTML = '';

  // Llamar a la función previemante creada 'buildToDos' pasándole como argumento el array toDoItems
...
function addToDo() {
  ...
  // Crear un nuevo ToDo usando la clase ToDo y pasándole el valor del input 'toDoInput' como parámetro
  const input = document.querySelector('#toDoInput');
  const value = input.value;
  const newTodo = new ToDo(value);

  // Agregar el objeto ToDo recién creado al array toDoItems
  toDoItems.push(newTodo);

  // Setear el valor del input toDoInput como un string vacio ("") (Esto se realiza para que en la vista se borre lo que se encontraba escrito)
  input.value = '';

  // Llamar a la función displayToDos para que se actualicen los toDos mostrados en pantalla
  displayToDos()
}
